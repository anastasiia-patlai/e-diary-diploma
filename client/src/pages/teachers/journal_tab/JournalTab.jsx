...
const JournalTab = ({ databaseName, isMobile, onOpenGradebook }) => {
    ...
    const loadTeacherJournals = async () => {
        ...
        try {
            ...
            // #8 — Deduplicate by subject + group + subgroup.
            // Multiple schedule entries for the same subject/group/subgroup (different day/time)
            // should appear as ONE journal. We keep the first scheduleId encountered per unique key,
            // so the gradebook loads that schedule and fetches all grades for it.
            const seen = new Map(); // key -> journal entry

            response.data.forEach(lesson => {
                const groupName = lesson.group?.name || '';
                const subgroup = lesson.subgroup || 'all';
                // Unique key: subject + groupId + subgroup (ignore day/time)
                const key = `${lesson.subject}__${lesson.group?._id}__${subgroup}`;

                if (!seen.has(key)) {
                    let classNumber = 999;
                    const match = groupName.match(/^(\d+)/);
                    if (match) classNumber = parseInt(match[1], 10);

                    seen.set(key, {
                        _id: lesson._id,       // first scheduleId — used to open gradebook
                ...
                // If already seen, skip — same journal
            });

            const formattedJournals = [...seen.values()];
        ...
        return (
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '400px' }}>
                <div style={{ textAlign: 'center' }}>
                    ...
                    <p style={{ marginTop: '10px', color: '#6b7280' }}>Завантаження журналів...</p>
                </div>
            </div>
        );
    }
    ...
    if (journals.length === 0) {
        return (
            <div style={{
                ...
                <h3 style={{ color: '#374151', marginBottom: '8px' }}>Немає доступних журналів</h3>
                <p style={{ color: '#6b7280' }}>У вас поки немає уроків у розкладі</p>
            </div>
        );
    }
    ...
    return (
        <div>
            <h3 style={{ fontSize: isMobile ? '18px' : '24px', marginBottom: '24px', color: '#374151' }}>
                Мої журнали
            </h3>
                    ...
                    <div key={subject} style={{
                        ...
                        <div style={{
                            ...
                            <div style={{
                                ...
                                <span style={{ fontWeight: '600', fontSize: isMobile ? '16px' : '18px', color: '#1f2937' }}>
                                    {subject}
                                </span>
                            </div>
                            <span style={{
                            ...
                            {groupedJournals[subject].map(journal => (
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        onMouseOver={e => { e.currentTarget.style.backgroundColor = 'rgba(85, 160, 165, 1)'; e.currentTarget.style.transform = 'translateY(-1px)'; }}
                                        onMouseOut={e => { e.currentTarget.style.backgroundColor = 'rgba(105, 180, 185, 1)'; e.currentTarget.style.transform = 'translateY(0)'; }}
                                    >
                                        <FaBookOpen />
